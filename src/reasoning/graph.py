...
def create_reasoning_graph() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        ...
        {
            "tool_node": "tool_node",
            "mcts_evaluate": "mcts_evaluate"
        }
    )
    
    # Tool -> Expand (Loop back to use results)
    # Actually, after Tool, we want to Continue Expanding from the same node?
    # Yes, we updated 'selected_node_id' to the search node.
    # So we go back to 'mcts_expand' to generate the NEXT step (using search results).
